...
def get_lorenz_data(n_ics, noise_strength=0, A=0.):
    ...
    data = generate_lorenz_data(ics, t, input_dim, linear=False, normalization=np.array([1/40,1/40,1/40]), A=A)
...
def generate_lorenz_data(ics, t, n_points, linear=True, normalization=None,
    ...
    dz = np.zeros(z.shape)
    ddz = np.zeros(z.shape)
    u = np.zeros((n_ics,n_steps,1))
    for i in range(n_ics):
        z[i], dz[i], ddz[i], u[i] = simulate_lorenz_with_control(ics[i], t, sigma=sigma, beta=beta, rho=rho, A=A)
    # NEW: simulate lorenz with control
...
def simulate_lorenz_with_control(z0, t, sigma=10., beta=8/3, rho=28., A=0., w=10.):
    """
    Simulate the Lorenz dynamics with input.
    ...
    Returns:
        z, dz, ddz - Arrays of the trajectory values and their 1st and 2nd derivatives.
    """
    f = lambda z,t : [sigma*(z[1] - z[0]) + A*np.sin(w*t), z[0]*(rho - z[2]) - z[1], z[0]*z[1] - beta*z[2]]
    df = lambda z,dz,t : [sigma*(dz[1] - dz[0]) + A*w*np.cos(w*t),
                          dz[0]*(rho - z[2]) + z[0]*(-dz[2]) - dz[1],
                          dz[0]*z[1] + z[0]*dz[1] - beta*dz[2]]
    u = lambda t: A*np.sin(w*t)
    ...
    for i in range(t.size):
        dz[i] = f(z[i],dt*i)
        ddz[i] = df(z[i], dz[i], dt*i)
    return z, dz, ddz, u(t).reshape(-1,1)