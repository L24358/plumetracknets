...
dic_regime = bcs.pklload("regimes", f"regime_agent={specify+1}.pkl")
...
for i in range(len(C)):
    angle, theta, epi, t = angles[i], fp_thetas[i], epis[i], ts[i]
...
vis.simpleaxis(ax)
plt.scatter(angles[idxs["tracking"]], fp_thetas[idxs["tracking"]], s=1, alpha=0.3, color="lightsteelblue")
plt.scatter(angles[idxs["recovery"]], fp_thetas[idxs["recovery"]], s=1, alpha=0.3, color="grey")
plt.scatter(angles[idxs["lost"]], fp_thetas[idxs["lost"]], s=1, alpha=0.3, color="pink")
plt.plot(wind_grid_mean, mean, color="b", marker='o')
plt.plot([-np.pi/2, -np.pi/2], [min(fp_thetas[idxs["tracking"]]), max(fp_thetas[idxs["tracking"]])], "k--") 
plt.plot([np.pi/2, np.pi/2], [min(fp_thetas[idxs["tracking"]]), max(fp_thetas[idxs["tracking"]])], "k--")
plt.xlabel("wind angle"); plt.ylabel("agent angle $a(h^*)$")
vis.savefig(figname=f"fpangle_windangle_regime.png")
